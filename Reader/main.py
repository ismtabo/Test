...
def run(test):
    ...
    :param test:
    """
    list_msg = ["Introduzca su respuesta o '0' para no contestar: ",
                "Introduzca sus respuestas separadas por comas(puede haber ninguna respuesta isCorrectAnswer) o '0' para no contestar: "]
    for qkey in range(1, test.getNumQuestions() + 1):
        ...
        print pregunta.getWording()

        for akey in sorted(pregunta.getAnswers().keys()):
            print "{}) {}".format(akey, pregunta.respuestas[akey][1])

        msg = list_msg[(int)(multiple)]
        user_answer = raw_input(msg)
        while (not pregunta.isValidAnswer(user_answer) and not user_answer == str(0)):
